        ...
        if($_POST){
            ...
            if(!$eposta||!$sifre){
                ...
                if(!filter_var($eposta,FILTER_VALIDATE_EMAIL)){
                    echo '<div class="alert alert-danger">E posta formatı yanlış girdiniz</div>';
                }else{
                    $girisyap=$db->prepare("SELECT * FROM yoneticiler 
                        WHERE eposta=:e AND sifre=:s LIMIT :lim");
                    ...
                    if($girisyap->rowCount()){
                        ...
                        $_SESSION['id']=$row->id;
                        echo '<div class="alert alert-success">Başarılı Bir Şekilde Giriş Yaptınız Bekleyiniz...</div>';
                        header('refresh:3;url=index.php');


                    }else{

                        echo '<div class="alert alert-danger">Böyle bir yönetici bulunmamaktadır</div>';
                    }
                }
            }
        }
